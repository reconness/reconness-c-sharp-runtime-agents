...
public class AgentServiceSkipTest
{
    ...
    public async Task Run_Skip_Subdomain_Include_Command_Async()
    {
        ...
        var payload = queueData
            .Replace("{{channel}}", channel)
            .Replace("{{command}}", agent.Command)
            .Replace("{{subdomain}}", subdomains[0].Name);

        // act
        // first subdomain skipped www.myrootdomain.com
        await agentService.RunAsync(unitOfWork, payload);
        ...
        payload = queueData
            .Replace("{{channel}}", channel)
            .Replace("{{command}}", agent.Command)
            .Replace("{{subdomain}}", subdomains[1].Name);

        // act
        // first subdomain not skipped www.myrootdomain1.com
        await agentService.RunAsync(unitOfWork, payload);

        subdomainSaved = await unitOfWork.Repository<Subdomain>().GetByCriteriaAsync(a => a.Name == "www.myrootdomain1.com");
        subdomain1Saved = await unitOfWork.Repository<Subdomain>().GetByCriteriaAsync(a => a.Name == "www1.myrootdomain.com");
        agentRunnerSaved = await unitOfWork.Repository<AgentRunner>().GetByCriteriaAsync(a => a.Channel == channel);
        agentRunnerCommandSaved = await (unitOfWork.Repository<AgentRunnerCommand>().GetAllQueryableByCriteria(a => a.AgentRunnerId == agentRunner.Id)).OrderBy(c => c.CreatedAt).LastOrDefaultAsync();
        agentRunnerCommandOutputSaved = await unitOfWork.Repository<AgentRunnerCommandOutput>().GetByCriteriaAsync(a => a.AgentRunnerCommandId == agentRunnerCommandSaved!.Id);
        ...
        agentRunnerCommandSaved.Should().NotBeNull();
        agentRunnerCommandSaved!.Status.Should().Be(Domain.Core.Enums.AgentRunnerCommandStatus.SUCCESS);
        agentRunnerCommandSaved!.Command.Should().Be("ping www.myrootdomain1.com");
        agentRunnerCommandSaved!.Server.Should().Be(1);
        agentRunnerCommandSaved!.Number.Should().Be(5);
